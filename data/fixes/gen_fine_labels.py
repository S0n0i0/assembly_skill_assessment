...
modality = str(sys.argv[1]) if len(sys.argv) > 1 else None
split = str(sys.argv[2]) if len(sys.argv) > 2 else None
...
if modality is not None and split is not None:
    ...
    actions = pd.read_csv(
        anno_path + "/actions.csv",
        delimiter=",",
        header=0,
        names=first_lines["actions"],
    )
    # create file category.txt with action_class
    with open(out_path + "/category.txt", "w") as f:
        for _, a in actions.iterrows():
            f.write(f"{a.action_cls}\n")
