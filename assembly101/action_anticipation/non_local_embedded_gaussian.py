...
class NONLocalBlock1D(nn.Module):
    def __init__(self, args, recent_dim, spanning_dim, latent_dim):
        ...
        if self.scale:
            self.scale_factor = torch.tensor(
                [self.latent_dim**self.scale_factor], requires_grad=True
            ).to("cuda")
    ...
    def forward(self, x_past, x_curr):
        ...
        theta_phi = torch.matmul(
            theta_x, phi_x
        )  # (NxCxnum_feat1) X (NxCxnum_feat2) = (NxCxC)
