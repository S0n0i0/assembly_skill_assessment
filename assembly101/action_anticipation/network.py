...
from assembly101.action_anticipation.non_local_embedded_gaussian import NONLocalBlock1D
import torch
...
class CouplingBlocks(nn.Module):
    def __init__(self, args, recent_dim, in_dim_past):
        ...
        if self.past_attention:
            self.NLB_past = NONLocalBlock1D(
                args, self.in_dim_past, self.in_dim_past, self.latent_dim
            )
        self.NLB_recent = NONLocalBlock1D(
            args, self.recent_dim, self.in_dim_past, self.latent_dim
        )

        self.fc_recent = nn.Sequential(
            nn.Linear(
                in_features=2 * self.recent_dim * self.video_feat_dim,
        ...
        self.fc_context = nn.Sequential(
            nn.Linear(
                in_features=self.in_dim_past * self.video_feat_dim
                + 2 * self.recent_dim * self.video_feat_dim,
                out_features=self.linear_dim,
            ),
            nn.ReLU(),
            nn.Dropout(self.dropout_rate),
...
class Network(nn.Module):
    def __init__(self, args):
        ...
        self.cls_act1 = nn.Sequential(
            nn.Linear(in_features=2 * self.linear_dim, out_features=self.n_classes)
        )
        self.cls_act2 = nn.Sequential(
            nn.Linear(in_features=2 * self.linear_dim, out_features=self.n_classes)
        )
        self.cls_act3 = nn.Sequential(
            nn.Linear(in_features=2 * self.linear_dim, out_features=self.n_classes)
        )
        self.cls_act4 = nn.Sequential(
            nn.Linear(in_features=2 * self.linear_dim, out_features=self.n_classes)
        )
        ...
        if args.add_verb_loss:
            self.cls_verb1 = nn.Sequential(
                nn.Linear(in_features=2 * self.linear_dim, out_features=args.verb_class)
            )
            self.cls_verb2 = nn.Sequential(
                nn.Linear(in_features=2 * self.linear_dim, out_features=args.verb_class)
            )
            self.cls_verb3 = nn.Sequential(
                nn.Linear(in_features=2 * self.linear_dim, out_features=args.verb_class)
            )
            self.cls_verb4 = nn.Sequential(
                nn.Linear(in_features=2 * self.linear_dim, out_features=args.verb_class)
            )

        if args.add_object_loss:
            self.cls_object1 = nn.Sequential(
                nn.Linear(
                    in_features=2 * self.linear_dim, out_features=args.object_class
                )
            )
            self.cls_object2 = nn.Sequential(
                nn.Linear(
                    in_features=2 * self.linear_dim, out_features=args.object_class
                )
            )
            self.cls_object3 = nn.Sequential(
                nn.Linear(
                    in_features=2 * self.linear_dim, out_features=args.object_class
                )
            )
            self.cls_object4 = nn.Sequential(
                nn.Linear(
                    in_features=2 * self.linear_dim, out_features=args.object_class
                )
            )
    ...
    def forward(self, spanning_snippets, recent_snippets):
        out_tab_recent1, out_tab_past1 = self.TAB1(
            spanning_snippets, recent_snippets[0]
        )
        out_tab_recent2, out_tab_past2 = self.TAB2(
            spanning_snippets, recent_snippets[1]
        )
        out_tab_recent3, out_tab_past3 = self.TAB3(
            spanning_snippets, recent_snippets[2]
        )
        out_tab_recent4, out_tab_past4 = self.TAB4(
            spanning_snippets, recent_snippets[3]
        )
