    ...
    def step(self, loss=None, acc=None, criterion=lambda x1, x2: x1 or x2):
        if loss is None:
            ...
            better_loss = (loss < self.lowest_loss) and (
                (self.lowest_loss - loss) / self.lowest_loss > 0.01
            )
        if acc is None:
...
def recog_file(filename, ground_truth_path, overlap, background_class_list):
    # read ground truth
    gt_file = ground_truth_path.replace("\\", "/") + re.sub(
        ".*/", "/", filename.replace("\\", "/")
    )
    ...
    with open(filename, "r") as f:
        recog_content = f.read().split("\n")[
            0:-1
        ]  # framelevel recognition is in 6-th line of file
        f.close()
    ...
    for s in range(len(overlap)):
        tp1, fp1, fn1 = f_score(
            recog_content, gt_content, overlap[s], background_class_list
        )
        tp_arr.append(tp1)
        fp_arr.append(fp1)
        fn_arr.append(fn1)
...
def calculate_mof(ground_truth_path_name, prediction_path, background_class):
    ...
    for filename in filelist:
        correct, frames, tp_arr, fp_arr, fn_arr, edit_score_value = recog_file(
            filename, ground_truth_path_name, overlap, background_class
        )
        n_correct += correct
        n_frames += frames
...
class PostProcess(nn.Module):
    ...
    def dump_to_directory(self, path, suffix="_gt"):
        ...
        for video_id, video_value in self.results_dict.items():
            ...
            label_name_arr = [
                self.labels_dict_id2name[i.item()]
                for i in pred_value[: label_count.item()]
            ]
            new_label_name_expanded = (
                []
            )  # np.empty(len(recog_content), dtype=np.object_)
    ...
    def forward(self, outputs, video_names, framewise_labels, counts):
        ...
        Parameters:
            ...
            :param clip_length:
        """
        for output, vn, framewise_label, count in zip(
            outputs, video_names, framewise_labels, counts
        ):
...
class PostProcess_test(nn.Module):
    ...
    def dump_to_directory(self, path, suffix="_gt"):
        ...
        for video_chunk_id, video_value in self.results_dict.items():
            video_id, chunk_id = (
                video_chunk_id.split("@")[0],
                video_chunk_id.split("@")[1],
            )
            assert video_value[1] * video_value[3] >= video_value[4]
            upped_pred_logit = self.upsample_video_value(
                video_value[0][:, : video_value[1]], video_value[4], video_value[3]
            ).unsqueeze(0)
            if video_id == prev_vid_id:
                ...
                all_pred_value = (
                    upped_pred_logit  # With refinement softmax has to be added
                )
    ...
    def forward(
        ...
        for output, vn, framewise_label, count, chunk_size, chunk_id, vid_len in zip(
            outputs,
            video_names,
            framewise_labels,
            counts,
            ...
            if key in self.results_dict:
                ...
                prev_tensor, prev_count, prev_gt_labels, chunk_size, pre_vid_len = (
